  ...
  <div class="q-pa-md row justify-center items-center q-gutter-md">
    <q-card class="login-card">
      <q-card-section>
        ...
        <div class="text-h4 text-center">Registro</div>
        <div class="text-subtitle2 text-center">Completa los datos</div>
      </q-card-section>
      ...
      <q-card-section>
        ...
        >
          <template v-slot:error>
            <span v-for="error of v$.password.$errors" :key="error.$uid">
              {{ error.$message }}<br v-if="v$.password.$errors.length > 1"
            /></span>
          </template>
        </q-input>
        <q-input
          ...
          class="q-my-md"
          :error="v$.password2.$invalid"
          bottom-slots
        >
          <template v-slot:error>
            <span v-for="error of v$.password2.$errors" :key="error.$uid">
              {{ error.$message }}</span
            >
          </template>
        </q-input>
        <div class="text-center text-body2">
          ...
          <router-link to="/user/login">aqui</router-link>
        </div>
      </q-card-section>
      <q-separator />
      <q-card-actions align="center">
        <q-btn flat @click="tryRegister">Registro</q-btn>
      </q-card-actions>
    </q-card>
  </div>
</template>
...
export default {
    ...
    return {
      ...
      password2: {
        required: helpers.withMessage(
          "Necesitas repetir la contraseña",
          required
        ),
  ...
  methods: {
    ...mapActions(useAuth, ["registerUser"]),
    async tryRegister() {
      this.$q.loading.show();
